...
export const getProductData = async (dashboard) => {
  const productData = [];
  const productCount = await dashboard.productLocator.count();

  for (let j = 0; j < productCount; j++) {
    const title = await dashboard.productLocator.nth(j).locator(dashboard.productTitle).textContent();
    const imgSrc = await dashboard.productLocator.nth(j).locator(dashboard.productImg).getAttribute("src");
    const desc = await dashboard.productLocator.nth(j).locator(dashboard.productDesc).textContent();
    const price = await dashboard.productLocator.nth(j).locator(dashboard.productPrice).textContent();
    const cartButton = dashboard.productLocator.nth(j).locator(dashboard.productCartBtn);
...
export const collectAllProductsData = async (page, dashboard) => {
  const allProductsData = [];
  const paginationButtons = await dashboard.paginationLocator.locator("button").all();
